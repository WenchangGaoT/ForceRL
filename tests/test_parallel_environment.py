...
env = suite.make('TrainPrismaticEnv', **env_kwargs) 
obs = env.reset() 
...
def test_function(id):
    print(f"Task {id} is running on thread: {threading.current_thread().name}")
    time.sleep(2)  # Simulate a time-consuming task
    print(f"Task {id} completed on thread: {threading.current_thread().name}")
