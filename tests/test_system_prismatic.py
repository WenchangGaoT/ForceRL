...
if len(joint_poses ) >1:
    ...
    joint_pose = joint_poses[0]
    joint_direction = joint_directions[0]
joint_poses += object_pos - np.array([reset_x_range[0], reset_y_range[0], 0])
joint_pose += object_pos - np.array([reset_x_range[0], reset_y_range[0], 0])
print("-----------------")
...
# scale the proposals
print("raw grasp pose: ", top_k_pos_wf[2])
# top_k_pos_wf = np.array(top_k_pos_wf) * 3

print("scaled grasp pose: ", top_k_pos_wf[2])

# top_k_pos_wf = top_k_pos_wf + object_pos - np.array([reset_x_range[0], reset_y_range[0], 0])
top_k_pos_wf = top_k_pos_wf + object_pos

# top_k_pos_wf = top_k_pos_wf + np.array([0, 0.1, 0.])

grasp_pos = top_k_pos_wf[2]
grasp_quat = top_k_quat_wf[2]
