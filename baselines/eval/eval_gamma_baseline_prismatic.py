from env.baseline_prismatic_eval_env import BaselineEvalPrismaticEnv
...
def eval_gamma_prismatic(experiment_name, run_id, object_type="cabinet", success_threshold=0.7):
    ...
    env_name = "BaselineEvalPrismaticEnv"


    trials_per_object = 20
    available_objects = BaselineEvalPrismaticEnv.available_objects()[object_type]
    
    baselines_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    eval_results_dir = os.path.join(baselines_dir, f"eval_results/gamma_prismatic_{experiment_name}_{run_id}")
    ...
    for obj_name in available_objects: 
        ...
        env:BaselineEvalPrismaticEnv = suite.make(
            env_name, 
            **env_kwargs
        )
        env._set_drawer_friction(3.0)
        ...
        for exp_num in range(trials_per_object):
            ...
            # select the first joint with joint_type = 1
            ...
            if joint_pose_selected is None:
                # randomly select a joint
                print("No joint with joint_type = 1 found. Randomly selecting a joint")
                joint_idx = np.random.randint(0, len(joint_types))
                joint_pose_selected = joint_poses[joint_idx]
                joint_direction_selected = joint_directions[joint_idx]
                joint_direction_selected = joint_direction_selected / np.linalg.norm(joint_direction_selected)
            ...
            # robot is always pulling, so adjust the direction so that it is pointing towards the robot
            vector_start = h_point
            vector_end = h_point + h_direction
            robot_base_pos = env.robot_base_xpos

            if np.linalg.norm(vector_end - robot_base_pos) < np.linalg.norm(vector_start - robot_base_pos):
                h_direction_adjusted = -h_direction
            else:
                h_direction_adjusted = h_direction
            ...
            for i in range(990):
                # action is the direction of the joint

                # robot is always pulling

                action = h_direction_adjusted
...
if __name__ == "__main__":
    experiment_name = "baseline_prismatic_trail"
    object_type = "trashcan"
    success_threshold = 0.7
    # delete_camera_frame_grasp_proposals()
    for i in range(8,10):
        eval_gamma_prismatic(experiment_name, i, object_type=object_type, success_threshold=success_threshold)
        delete_camera_frame_grasp_proposals()
        time.sleep(1)
